#!/usr/bin/env python3
# Python Basis Tool (pbt) script for reading in Molpro internal (libmol) format and writing to (the same) Molpro internal format
#
# Please check $molpro.py for more info and limitations.
#
import sys
import util
...
if (len(sys.argv) < 2):
    print("Usage: molint2molint.py infile [outfile]")
...
for entry in set:
    print(entry.info())
#
molpro.WriteInt(set,precis,outfile)
outfile.close()
